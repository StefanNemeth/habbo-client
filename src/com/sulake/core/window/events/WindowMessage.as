
package com.sulake.core.window.events
{
    import com.sulake.core.window.IWindow;

    public class WindowMessage extends WindowEvent 
    {

        private static const WE_MESSAGE:String = "WE_MESSAGE";
        private static const POOL:Array = [];

        public var message:String;

        public function WindowMessage()
        {
            _type = WE_MESSAGE;
        }
        public static function allocate(_arg_1:String, _arg_2:IWindow, _arg_3:IWindow, _arg_4:Boolean=false):WindowEvent
        {
            var _local_5:WindowMessage = (((POOL.length)>0) ? POOL.pop() : new (WindowMessage)());
            _local_5.message = _arg_1;
            _local_5._window = _arg_2;
            _local_5._related = _arg_3;
            _local_5.set = _arg_4;
            _local_5._recycled = false;
            _local_5._SafeStr_9532 = POOL;
            return (_local_5);
        }

        override public function clone():WindowEvent
        {
            return (allocate(this.message, window, related, cancelable));
        }
        override public function toString():String
        {
            return ((((((((("WindowLinkEvent { type: " + _type) + " message: ") + this.message) + " cancelable: ") + set) + " window: ") + _window) + " }"));
        }

    }
}//package com.sulake.core.window.events

// WindowEvent = "_-Jh" (String#2085, DoABC#2)
// WindowMessage = "_-31u" (String#7344, DoABC#2)
// allocate = "_-08G" (String#14374, DoABC#2)
// related = "_-2Pu" (String#20039, DoABC#2)
// _related = "_-2q8" (String#21086, DoABC#2)
// _recycled = "_-2vB" (String#1963, DoABC#2)
// _SafeStr_9532 = "_-Tf" (String#23392, DoABC#2)
// WE_MESSAGE = "_-370" (String#21768, DoABC#2)


